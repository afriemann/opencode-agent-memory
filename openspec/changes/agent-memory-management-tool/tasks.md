...
- [ ] 2.1 Add `cmdCorrect(agent, project, patchJson)` function to `src/memory.js` — parses the patch JSON, reads the current `hot_state` row inside a transaction, merges supplied fields onto the row (absent fields keep current values, using `EMPTY_RECORD` defaults for a cold start), UPSERTs with `updated_at = current_updated_at + 1`, and does NOT delete signals or advance the watermark
...
- [ ] 4.2 Implement `memory_inspect` tool: `execute({ }, context)` calls `spawnMemory($, ['inspect', resolvedAgent, context.directory])`, parses the JSON result, and returns a `ToolResult`; wraps body in try/catch and returns an informative error result on failure
- [ ] 4.3 Implement `memory_correct` tool: `execute({ patch }, context)` serialises the `patch` object to JSON and calls `spawnMemory($, ['correct', resolvedAgent, context.directory, patchJson])`; returns a `ToolResult`; wraps body in try/catch
