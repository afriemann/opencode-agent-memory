...
The system SHALL attempt to read `~/.config/opencode/agent-memory.jsonc` (resolving `~` via `os.homedir()`) synchronously at module load time and use its values as defaults for `targetAgent`, `distilMinIntervalMs`, and `distillerModel`.

#### Scenario: Config file present with valid values

- **WHEN** `~/.config/opencode/agent-memory.jsonc` exists and contains valid JSONC with one or more of the keys `targetAgent`, `distilMinIntervalMs`, or `distillerModel`
...
- **WHEN** `MEMORY_TARGET_AGENT=navigator` is set in the environment AND `~/.config/opencode/agent-memory.jsonc` contains `"targetAgent": "engineer"`
- **THEN** the resolved `targetAgent` is `navigator` (env var wins)

#### Scenario: Only config file set

- **WHEN** `MEMORY_TARGET_AGENT` is not set in the environment AND `~/.config/opencode/agent-memory.jsonc` contains `"targetAgent": "planner"`
- **THEN** the resolved `targetAgent` is `planner`

#### Scenario: Neither env var nor config file

- **WHEN** `MEMORY_TARGET_AGENT` is not set AND the config file is absent or does not include `targetAgent`
- **THEN** the resolved `targetAgent` is `engineer` (hardcoded default)
...
Validation rules:
- `targetAgent` — MUST be a non-empty string.
...
- **WHEN** `~/.config/opencode/agent-memory.jsonc` contains `"distilMinIntervalMs": "fast"` and `"targetAgent": "builder"`
- **THEN** the plugin emits a `[agent-memory]` warning for `distilMinIntervalMs` and uses `60000`; `targetAgent` is resolved to `builder` (other key unaffected)
...
#### Scenario: `targetAgent` is empty string

- **WHEN** `~/.config/opencode/agent-memory.jsonc` contains `"targetAgent": ""`
- **THEN** the plugin emits a `[agent-memory]` warning for `targetAgent` and uses `engineer`
...
The system SHALL silently ignore any key in `~/.config/opencode/agent-memory.jsonc` that is not one of `targetAgent`, `distilMinIntervalMs`, or `distillerModel`. No warning is emitted for unknown keys.
